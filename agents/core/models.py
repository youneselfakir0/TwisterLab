...
"""
from sqlalchemy import Column, Integer, String, DateTime, JSON, Float, Enum as SQLEnum, Text
...
class Ticket(Base):
    ...
    priority: TicketPriority = Column(SQLEnum(TicketPriority), nullable=False, default=TicketPriority.MEDIUM)
...
class AgentLog(Base):
    ...
    ticket_id: int = Column(Integer, nullable=True, index=True)  # FK to tickets (not enforced for flexibility)
    ...
    timestamp: datetime = Column(DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), index=True)
