...
from agents.core.models import Ticket, AgentLog, SystemMetrics, TicketStatus, TicketPriority
...
class TicketRepository:
    ...
    async def create(
        ...
        logger.info(f"Created ticket #{ticket.id}: {category or 'uncategorized'} ({priority.value})")
        return ticket
...
class AgentLogRepository:
    ...
    async def get_recent_logs(self, agent_name: Optional[str] = None, limit: int = 50) -> List[AgentLog]:
