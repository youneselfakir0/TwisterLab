...
class BrowserAgent(UnifiedAgentBase):
    ...
    def __init__(self):
        ...
        self.tools = (
            self._define_tools()
        )  # Keep tools definition for external consumption if needed
        ...
        return [
            ...
            },
            {
                ...
                "parameters": {
                    "selector": {
                        "type": "string",
                        "description": "The CSS selector of the form field",
                    },
            ...
            },
            {
                ...
                "parameters": {
                    "table_selector": {
                        "type": "string",
                        "description": "CSS selector of the table to extract data from",
                    }
                },
            },
            {
            ...
            },
            {
                ...
                "parameters": {
                    "action": {
                        "type": "string",
                        "description": "Action to perform (click, fill, etc.)",
                    },
                    "selectors": {
                        "type": "array",
                        "description": "Array of CSS selectors to apply the action to",
                    },
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "login_button_selector": {
                        "type": "string",
                        "description": "Selector for login button",
                    },
        ...
        try:
            if task == "open_browser":
                ...
                try:
                    ...
                    raise ImportError(
                        "Playwright not installed. Please run 'pip install playwright' and 'playwright install'."
                    )
                ...
                self._add_to_audit_log(
                    "open_browser", {"headless": self.headless, "target_url": self.target_url}
                )
                ...
                if not table_selector:
                    raise ValueError("Table selector is required for extract_table_data.")
                ...
                for row in rows:
                    ...
                    if row_data:
                        table_data.append([text.strip() for text in row_data])
                ...
                if not action or not selectors:
                    raise ValueError("Action and selectors are required for perform_bulk_action.")
                results = []
                ...
                self._add_to_audit_log(
                    "perform_bulk_action", {"action": action, "selectors": selectors}
                )
    ...
    def get_last_action_time(self):
        return self.audit_log[-1]["timestamp"] if self.audit_log else None

    def get_action_count(self):
        return len(self.audit_log)

    def get_action_by_time(self, timestamp):
        return next((a for a in self.audit_log if a["timestamp"] == timestamp), None)

    def get_action_by_type(self, action_type):
        return [a for a in self.audit_log if a["action"] == action_type]

    def get_action_by_status(self, status):
        return [a for a in self.audit_log if a.get("status") == status]

    def get_action_by_message(self, message):
        return [a for a in self.audit_log if a.get("message") == message]

    def get_action_by_parameters(self, parameters):
        return [a for a in self.audit_log if a.get("parameters") == parameters]

    def get_action_by_timestamp_range(self, start_time, end_time):
        return [a for a in self.audit_log if start_time <= a["timestamp"] <= end_time]

    def get_action_by_time_range(self, start_time, end_time):
        return self.get_action_by_timestamp_range(start_time, end_time)
