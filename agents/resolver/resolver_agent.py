...
class ResolverAgent(TwisterAgent):
    ...
    async def execute(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"[{self.name}] Resolution failed for ticket {ticket_id}: {e}", exc_info=True)
            
            # Automatic escalation on critical failure
            escalation_result = await self._escalate(
                ticket_id,
                classification,
                {"error": str(e), "original_task": task}
            )
    ...
    async def _update_ticket_status(
        ...
        try:
            ...
            logger.info(f"[ResolverAgent] Would update ticket {ticket_id} status (TicketService TODO)")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                health["checks"]["desktop_commander"] = "healthy" if response.status_code == 200 else "unhealthy"
        except Exception as e:
