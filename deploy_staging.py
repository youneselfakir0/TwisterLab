...
def run_tests(skip: bool = False):
    ...
    returncode, stdout, stderr = run_command(["pytest", "tests/", "-v", "--tb=short"], check=False)
...
def pull_images(pull: bool = False):
    ...
    returncode, stdout, stderr = run_command(
        ["docker-compose", "-f", "docker-compose.staging.yml", "pull"], check=False
    )
...
def stop_existing_containers():
    ...
    returncode, stdout, stderr = run_command(
        ["docker-compose", "-f", "docker-compose.staging.yml", "down"], check=False
    )
...
def build_images():
    ...
    returncode, stdout, stderr = run_command(
        ["docker-compose", "-f", "docker-compose.staging.yml", "build", "--no-cache"], check=False
    )
...
def start_services():
    ...
    returncode, stdout, stderr = run_command(
        ["docker-compose", "-f", "docker-compose.staging.yml", "up", "-d"], check=False
    )
...
def wait_for_health_checks():
    ...
    while elapsed < max_wait:
        returncode, stdout, stderr = run_command(
            ["docker-compose", "-f", "docker-compose.staging.yml", "ps"], check=False
        )
