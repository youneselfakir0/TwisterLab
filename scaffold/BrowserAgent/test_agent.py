...
from twisterlab.twisterlang.codec import build_message, encode_message_to_base64, decode_message_from_base64
import pytest
...
def test_browser_agent_functionality(browser_agent):
    ...
    result = browser_agent.execute_tool("create_browser_tool", {"target_url": "https://example.com"})
...
def test_browser_agent_error_handling(browser_agent):
    ...
    result = browser_agent.execute_tool("create_browser_tool", {"target_url": "invalid_url"})
    assert result.get("status") == "error"

def test_twisterlang_roundtrip(browser_agent):
    payload = {"twisterlang_version": "1.0", "correlation_id": "roundtrip-id", "data": {"k": "v"}}
