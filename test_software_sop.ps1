# Test ResolverAgent avec SOP SOFTWARE

$ApiUrl = "http://192.168.0.30:8000"

Write-Host "================================================" -ForegroundColor Cyan
Write-Host " TEST RESOLVERAGENT + SOP SOFTWARE_INSTALL" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""

# Ticket software - devrait declencher ResolverAgent avec SOP software_install
$ticket = @{
    ticket_id = "T-SOFT-001"
    title = "Need to install new software application"
    description = "User needs Python 3.11 installed on their workstation. Please install the latest version and verify it works."
} | ConvertTo-Json

Write-Host "Ticket Software:" -ForegroundColor Yellow
Write-Host "  ID: T-SOFT-001" -ForegroundColor White
Write-Host "  Probleme: Install Python software" -ForegroundColor White
Write-Host "  Attendu: ClassifierAgent -> ResolverAgent -> SOP software_install" -ForegroundColor Gray
Write-Host ""
...
try {
    ...
    foreach ($step in $result.workflow_results.steps) {
        ...
        if ($step.step -eq "resolution") {
            ...
            if ($step.result.result) {
                ...
                $sopSteps = $step.result.result.steps_executed
                if ($sopSteps -is [array]) {
                    foreach ($sopStep in $sopSteps) {
                ...
                if ($step.result.result.success_rate) {
                    Write-Host "    Taux succes: $([math]::Round($step.result.result.success_rate * 100))%" -ForegroundColor Cyan
                }
            }
        }
    ...
    Write-Host "================================================" -ForegroundColor Green
    Write-Host "WORKFLOW COMPLET!" -ForegroundColor Green
    Write-Host "================================================" -ForegroundColor Green
    Write-Host ""
