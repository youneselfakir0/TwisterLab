...
async def test_hybrid_prefers_azure_when_credentials_available():
    ...
    with patch.dict(
        ...
        assert hybrid.mode in [
            "azure",
            "local",
        ], f"Mode should be azure or local, got {hybrid.mode}"
...
async def test_hybrid_verify_token_delegates_correctly():
    ...
    with patch.dict(
        ...
        token = hybrid.local_auth.create_access_token(
            {"sub": "admin", "email": "admin@twisterlab.local", "roles": ["admin"]}
        )
...
def test_hybrid_mode_local_has_working_methods():
    ...
    with patch.dict(
        ...
        token = hybrid.create_access_token({"sub": "testuser", "roles": ["user"]})
