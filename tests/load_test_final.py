...
class LoadTester:
    ...
    def print_results(self, result: LoadTestResult):
        ...
        checks = [
            ("Throughput", result.throughput_per_sec >= 100, 
             f"{result.throughput_per_sec:.1f} >= 100 req/s"),
            ("p95 Latency", result.p95_latency_ms < 500,
             f"{result.p95_latency_ms:.1f}ms < 500ms"),
            ("p99 Latency", result.p99_latency_ms < 1000,
             f"{result.p99_latency_ms:.1f}ms < 1000ms"),
            ("Error Rate", result.error_rate_percent < 1.0,
             f"{result.error_rate_percent:.2f}% < 1%"),
            ("CPU Usage", result.avg_cpu_percent < 80,
             f"{result.avg_cpu_percent:.1f}% < 80%"),
            ("Memory Usage", result.avg_memory_percent < 80,
