...
class SecurityAuditor:
    ...
    def __init__(self):
        self.findings: Dict[str, Any] = {"bandit": {}, "safety": {}, "trivy": {}}
    ...
    def generate_report(self):
        ...
        if critical_high == 0:
            ...
            report.append(
                "⚠️  **MEDIUM RISK** - Some critical/high-severity issues require attention.\n"
            )
        else:
            report.append(
                "❌ **HIGH RISK** - Multiple critical/high-severity vulnerabilities detected.\n"
            )
        ...
        if self.findings["safety"].get("status") == "completed":
            ...
            report.append(
                f"**Vulnerable Packages:** {', '.join(safety['vulnerable_packages']) if safety['vulnerable_packages'] else 'None'}\n"
            )
            ...
            if safety["vulnerabilities"]:
                ...
                for i, vuln in enumerate(safety["vulnerabilities"][:5], 1):
                    if isinstance(vuln, dict):
                        report.append(
                            f"{i}. **{vuln.get('package', 'Unknown')}** {vuln.get('installed_version', '')}"
                        )
