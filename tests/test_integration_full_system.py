...
async def test_ticket_lifecycle_full_pipeline():
    ...
    logger.info(f"   Classifier Confidence: {monitoring_result['metrics']['classifier_confidence']:.1%}")
...
async def test_error_handling_and_failover():
    ...
    if classifier_low_confidence < 0.60:
        logger.warning(f"   ⚠️  ESCALATION TRIGGERED: Confidence {classifier_low_confidence:.1%} < 0.60 threshold")
...
async def test_monitoring_and_alerting():
    ...
    for alert in alerts:
        logger.info(f"   [{alert['severity']}] {alert['metric']}: {alert['value']} > {alert['threshold']}")
