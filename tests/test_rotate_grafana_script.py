...
def test_use_service_name_override():
	proc = run_ps_script(['scripts/rotate_grafana_secret.ps1', '-DryRun', '-ServiceName', 'grafana_test', '-RestartService'])
	assert proc.returncode == 0
	assert 'Using provided service name: grafana_test' in proc.stdout


def test_autodetect_single_candidate():
	proc = run_ps_script(['scripts/rotate_grafana_secret.ps1', '-DryRun', '-TestServiceList', 'grafana', '-RestartService'])
	assert proc.returncode == 0
...
def test_autodetect_prefers_port_3000():
	...
	proc = run_ps_script(['scripts/rotate_grafana_secret.ps1', '-DryRun', '-TestServiceList', 'grafanaA,grafanaB', '-TestServicePortMap', 'grafanaB:3000', '-RestartService'])
	assert proc.returncode == 0
