...
$datasourceConfig = @{
    name = "Prometheus"
    type = "prometheus"
    url = "http://192.168.0.30:9090"
    access = "proxy"
    isDefault = $true
} | ConvertTo-Json

try {
    $response = Invoke-WebRequest -Uri "http://192.168.0.30:3000/api/datasources/1" -Method PUT -Headers $headers -Body $datasourceConfig -TimeoutSec 30
    Write-Host "Prometheus datasource updated successfully!"
    Write-Host "Response: $($response.Content)"
} catch {
    Write-Host "Error updating datasource: $($_.Exception.Message)"
